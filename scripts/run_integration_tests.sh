...
if [[ ! -f "../main.py" || ! -f "../tests/integration/test_complete_integration.py" ]]; then
    print_status $RED "❌ Not in Memory Context Manager v2 directory"
    echo "Please run this script from the project root directory"
    exit 1
fi

# Move to project root if running from scripts directory
...
if [[ "$1" == "--verbose" || "$1" == "-v" ]]; then
    python3 tests/integration/test_complete_integration.py --verbose 2>&1 | tee "$log_file"
    test_exit_code=${PIPESTATUS[0]}
elif [[ "$1" == "--ci" ]]; then
    python3 tests/integration/test_complete_integration.py --ci --exit-on-failure 2>&1 | tee "$log_file"
    test_exit_code=${PIPESTATUS[0]}
else
    python3 tests/integration/test_complete_integration.py 2>&1 | tee "$log_file"
    test_exit_code=${PIPESTATUS[0]}
fi
